...
namespace Voba.Services
{
    ...
    public class SpoonacularEnrichmentService : IRecipeEnrichmentService
    {
            ...
            var mapped = parsed
                .Select(i => new ParsedIngredient(
                    ...
                    (i.Nutrition?.Nutrients ?? new())
                        .Select(n => new ParsedNutrient(n.Name, n.Amount))
                        .ToList()))
                .ToList();

            return ComputeEnrichment(mapped, servings);
        }
        ...
        internal readonly record struct ParsedNutrient(string? Name, decimal Amount);
            ...
            IReadOnlyList<ParsedIngredient> parsed, int servings)
        {
            ...
            decimal totalCents = parsed
                .Where(i => i.CostCents != null)
                .Sum(i => i.CostCents!.Value);
